...
- `/spread 1000000` — запросить у Altyn котировку именно на указанную сумму и рассчитать результат в RUB.
...
- Altyn запрашивается приватным `GET https://api.lk.altyn.one/website/arbitrage-rate/?amount_rub=<сумма>` с обязательным заголовком `X-Arbitrage-Token`. Токен берётся только из `ALTYN_ARBITRAGE_TOKEN` и не включается в URL или логи.
...
- Rapira bid и ask берутся из текущего публичного стакана USDT/RUB. Taker-комиссия уровня 0 каждый раз читается из `market/fee/page-query`; отсутствующие цены или комиссия не заменяются последней сделкой либо нулём.
...
Для `/spread n` бот запрашивает у Altyn отдельную котировку точно на `n` RUB и вычитает полученную `network_fee_usdt` из количества USDT перед продажей на Rapira. Повторные одновременные или последовательные запросы одной и той же суммы в течение 15 секунд используют отдельный для этой суммы in-memory cache; суммы не смешиваются. Новые некэшированные запросы Altyn начинаются не чаще одного раза в 10 секунд, чтобы общий поток оставался ниже минутного и суточного лимитов API; слишком частый пользовательский запрос отклоняется, а плановый сбор дожидается доступного слота. Расчёт использует свежий сохранённый стакан и динамическую комиссию Rapira, но не моделирует проскальзывание, изменение цены во время перевода или фактическое исполнение сделок.
