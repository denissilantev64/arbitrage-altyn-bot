...
Telegram-бот для мониторинга спреда USDT/RUB между Altyn и Rapira. Бот раз в пять минут получает обе стороны котировок, сохраняет проверенный снимок и комиссии в SQLite, выбирает лучшее из двух направлений после комиссий и отправляет результат по запросу или в 09:00 МСК по будням.
...
Постоянное меню содержит «Показать спред», «Рассчитать прибыль», динамическую кнопку подписки и «Поддержка». Кнопка «Поддержка» показывает ссылку на Telegram-аккаунт поддержки.
...
- комиссии покупки и продажи Altyn задаются обязательными переменными `ALTYN_BUY_FEE_PERCENT` и `ALTYN_SELL_FEE_PERCENT` в процентах, потому что API курсов их не возвращает; допустимый диапазон — `[0, 100)`, в текущем примере обе равны `0.00`;
- taker-комиссия Rapira динамически читается из публичной таблицы уровня 0 и сохраняется в каждом снимке.
...
- Даже успешный снимок считается устаревшим через шесть минут: это покрывает пятиминутный интервал обновления и время HTTP-запроса.
- Пока свежего полного снимка нет, пользователь получает безопасное сообщение о недоступности курсов.
- Если утренняя рассылка ожидает свежий снимок или незавершенную доставку, готовность проверяется раз в минуту независимо от пятиминутного сбора курсов.
...
Заполните все обязательные переменные из `.env.example`, затем запустите:
...
Создайте `/etc/arbitrage-altyn-bot/bot.env` только с пятью runtime-переменными из примера выше. Не копируйте workstation `.env`, потому что он также содержит реквизиты доступа к серверу.
