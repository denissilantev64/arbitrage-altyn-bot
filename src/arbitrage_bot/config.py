...
_TELEGRAM_USERNAME_PATTERN = re.compile(r"[A-Za-z0-9_]{5,32}\Z", flags=re.ASCII)
...
class Settings:
    ...
    def from_environment(cls, dotenv_path: Path | None = None) -> Settings:
        ...
        parsed_support_url = urlparse(support_url)
        support_username = parsed_support_url.path.removeprefix("/")
        if (
            parsed_support_url.scheme != "https"
            or parsed_support_url.netloc != "t.me"
            or parsed_support_url.path != f"/{support_username}"
            or parsed_support_url.params
            or parsed_support_url.query
            or parsed_support_url.fragment
            or _TELEGRAM_USERNAME_PATTERN.fullmatch(support_username) is None
        ):
